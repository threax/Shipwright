...
void Interface_Draw(GlobalContext* globalCtx) {
    ...
    bool isPaused = globalCtx->pauseCtx.state != 0;
    bool minimapVisible = !R_MINIMAP_DISABLED;
    ...
    if (pauseCtx->debugState == 0) {
        ...
        if (fullUi || gSaveContext.health != gSaveContext.healthCapacity || minimapVisible || isPaused) {
            HealthMeter_Draw(globalCtx);
        }

        func_80094520(globalCtx->state.gfxCtx);

        if (fullUi || minimapVisible || isPaused) {
            // Rupee Icon
        ...
        if (fullUi || gSaveContext.unk_13F0 > 0 || minimapVisible || isPaused) {
